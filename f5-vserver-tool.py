#!/usr/bin/python 
#
# Commandline utility to execute specific operations on biotronik f5 loadbalancer.py
#
...
def parseArgs():
	...
	parser = argparse.ArgumentParser(description="Lists vserver, pool and nodes of from specified f5 big ip systems.\n"
	                                             ...
	                                             + "If no configfile is passed, /etc/loadbalancer.py.conf is used.\n"
	...
	parser.add_argument("-c", "--config", dest="config", action="store",
	                    help="Path to configfile. Use -m to print sample config file."
	                         + "\nDefault location is /etc/loadbalancer.py.conf.")
	...
	# loadbalancer.py
	parser.add_argument("-lb", "--loadbalancer.py", dest="loadbalancer.py", action="store",
	                    help="Loadbalancer system.\n"
	                         + "This option overwrites config file options.")

	# device group
	parser.add_argument("-dg", "--devicegroup", dest="devicegroup", action="store",
	                    help="Device group of the specified loadbalancer.py.\n"
	                         + "This option overwrites config file options.")
...
def auth(loadbalancer, user, password):
	""" Authenticates on the specified loadbalancer.py"""
	try:
		global mgmt
